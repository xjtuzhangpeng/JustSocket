...
extern void tit_store_log(char * line, int index);

/* "user interface" functions */
static void dump_stream_format(AVFormatContext *ic, int i,
                               int index, int is_output) // 获取 stream 的信息 - zhangpeng
{
    char buf[256];
    ...
    av_log(NULL, AV_LOG_TIT, "TIT_Stream #%d:%d", index, i);
    ...
    av_log(NULL, AV_LOG_INFO, ": %s", buf); // 语音信息 - zhangpeng
    tit_store_log(buf, index);
