...
#define _READ_FROM_FILE_    0
#define _WRITE_TO_FILE_     0
extern size_t 
    Read_Tit_Buff(const char * filename, void *buf, size_t len);
...
{
  size_t ret = Read_Tit_Buff(ft->filename, buf, len);
#if _READ_FROM_FILE_
  ret = fread(buf, (size_t) 1, len, (FILE*)ft->fp);
...
extern size_t 
    Write_Tit_Buff(const char * filename, void const * buf, size_t len);
/* Write a buffer of data of length bytes.
 * Returns number of bytes written.
 */
size_t lsx_writebuf(sox_format_t * ft, void const * buf, size_t len)
{
  size_t ret = Write_Tit_Buff(ft->filename, buf, len);
#if _WRITE_TO_FILE_
