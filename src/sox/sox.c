...
static void play_file_info(sox_format_t * ft, file_t * f, sox_bool full) {
    ...
    if (ft->filename[0]) {
        ...
        if (strcmp(ft->filename, "-") == 0
                || (ft->handler.flags & SOX_FILE_DEVICE))
    ...
    sprintf(buffer, "  Channels: %u @ %u-bit", ft->signal.channels,
            ft->signal.precision);
    ...
    fprintf(output, "  Duration: %-13s",
            ft->signal.length ?
                    str_time((double) ws / ft->signal.rate) : "unknown");
...
static void display_file_info(sox_format_t * ft, file_t * f, sox_bool full) {
    ...
    fprintf(output, "Channels       : %u\n"
            "Sample Rate    : %g\n"
            "Precision      : %u-bit\n", ft->signal.channels, ft->signal.rate,
            ft->signal.precision);
...
static sox_bool can_segue(size_t i) {
    return files[i]->ft->signal.channels == files[i - 1]->ft->signal.channels
            && files[i]->ft->signal.rate == files[i - 1]->ft->signal.rate;
}

static int combiner_drain(sox_effect_t *effp, sox_sample_t * obuf,
        size_t * osamp) {
    input_combiner_t * z = (input_combiner_t *) effp->priv;
    size_t ws, s, i;
    size_t olen = 0;
...
static sox_bool is_pseudo_effect(const char *s) {
    if (s)
        if (strcmp("newfile", s) == 0 || strcmp("restart", s) == 0
                || strcmp(":", s) == 0)
            return sox_true;
    return sox_false;
} /* is_pseudo_effect */
...
static void parse_effects(int argc, char ** argv) {
    while (optstate.ind < argc) {
        ...
        if (eff_offset == user_effargs_size[eff_chain_count]) {
            ...
            lsx_revalloc(user_effargs[eff_chain_count],
                    user_effargs_size[eff_chain_count]);
...
static void read_user_effects(char const *filename) {
    ...
    if (!file) {
        lsx_fail("Cannot open effects file `%s': %s", filename,
                strerror(errno));
        exit(1);
    }
    ...
    while (fgets(s + pos, (int) (buffer_size - pos), file)) {
        ...
        if (len && s[pos + len - 1] == '\n')
            ...
            lsx_fail("Error reading effects file `%s' (not a text file?)",
                    filename);
            exit(1);
        }
    ...
    if (ferror(file)) {
        lsx_fail("Error reading effects file `%s': %s", filename,
                strerror(errno));
        exit(1);
    }
    fclose(file);
...
static void create_user_effects(void) {
    ...
    for (i = 0; i < num_effects; i++) {
        ...
        if (effp->handler.flags & SOX_EFF_DEPRECATED)
            ...
            lsx_warn("effect `%s' is experimental/incomplete",
                    effp->handler.name);
...
static void add_effects(sox_effects_chain_t *chain) {
    ...
    if (is_guarded
            && (do_guarded_norm || !(signal.mult && *signal.mult == 1))) {
        char *args[2];
        int no_guard = -1;
        args[0] = do_guarded_norm ? "-nh" : guard ? "-rh" : "-h";
        args[1] = norm_level;
        auto_effect(chain, "gain", norm_level ? 2 : 1, args, &signal,
                &no_guard);
        guard = 1;
    }

    if (i == nuser_effects[current_eff_chain] && !no_dither
            && signal.precision >
...
static int advance_eff_chain(void) {
    ...
    if (input_eof)
        ...
        while (nuser_effects[current_eff_chain] == 1
            ...
            if (strcmp("newfile", user_effargs[current_eff_chain][0].name)
                    == 0) {
                if (++current_eff_chain >= eff_chain_count)
                    return SOX_EOF;
                reuse_output = sox_false;
            } else if (strcmp("restart",
                    user_effargs[current_eff_chain][0].name) == 0)
                current_eff_chain = 0;
        }
...
static sox_bool since(struct timeval * then, double secs,
    ...
    ret =
            d
                    > ceil(
                            secs) || now.tv_usec - then->tv_usec + d * TIME_FRAC >= secs * TIME_FRAC;
    if (ret || always_reset)
        *then = now;
    return ret;
}
...
static char const * vu(unsigned channel) {
    ...
    int index =
            vu_dB < 2 * white ?
                    max(vu_dB / 2, 0) : min(vu_dB - white, red + white - 1);
...
static void display_status(sox_bool all_done) {
    ...
    if (all_done || since(&then, .1, sox_false)) {
        ...
        fprintf(stderr, "\rIn:%-5s %s [%s] Out:%-5s [%6s|%-6s] %s Clip:%-5s",
                lsx_sigfigs3p(percentage), str_time(read_time),
                str_time(left_time), lsx_sigfigs3((double) output_samples),
...
static void optimize_trim(void) {
    ...
    if (input_count == 1 && very_first_effchain && effects_chain->length > 1
        ...
        if (files[0]->ft->handler.seek && files[0]->ft->seekable) {
            ...
            if (offset
                    && sox_seek(files[0]->ft, offset, SOX_SEEK_SET)
                            == SOX_SUCCESS) {
...
static void calculate_combiner_signal_parameters(void) {
    ...
    for (i = 0; i < input_count; i++) {
        size_t j;
        for (j = 0;
                j < nuser_effects[current_eff_chain]
...
static void calculate_output_signal_parameters(void) {
    ...
    for (i = 0; i < input_count; i++) {
        known_length = known_length
                && files[i]->ft->signal.length != SOX_UNSPEC;
        if (combine_method == sox_concatenate)
    ...
    for (i = 0; i < nuser_effects[current_eff_chain]; i++)
        known_length = known_length
                && !(user_efftab[i]->handler.flags & SOX_EFF_LENGTH);
...
static int process(void) { /* Input(s) -> Balancing -> Combiner -> Effects -> Output */
    ...
    if (!effects_chain)
        effects_chain = sox_create_effects_chain(&combiner_encoding, &ofile->ft->encoding);
    ...
    if (very_first_effchain) {
        ...
        double d;
        gettimeofday(&now, NULL);
        d = now.tv_sec - load_timeofday.tv_sec
...
static void usage_effect(char const * name) {
    ...
    if (strcmp("all", name) && !sox_find_effect(name)) {
        ...
        for (i = 0; sox_effect_fns[i]; i++) {
            ...
            if (e && e->name
                    && (!strcmp("all", name) || !strcmp(e->name, name))) {
                printf("%s %s\n", e->name, e->usage ? e->usage : "");
                if (e->flags
                        & (SOX_EFF_DEPRECATED | SOX_EFF_ALPHA | SOX_EFF_INTERNAL))
                    putchar('\n');
...
static void read_comment_file(sox_comments_t * comments,
...
static struct lsx_option_t const long_options[] = { { "add-comment",
        lsx_option_arg_required, NULL, 0 }, { "buffer", lsx_option_arg_required,
        NULL, 0 }, { "combine", lsx_option_arg_required, NULL, 0 }, {
        "comment-file", lsx_option_arg_required, NULL, 0 }, { "comment",
        lsx_option_arg_required, NULL, 0 }, { "endian", lsx_option_arg_required,
        NULL, 0 }, { "input-buffer", lsx_option_arg_required, NULL, 0 }, {
        "interactive", lsx_option_arg_none, NULL, 0 }, { "help-effect",
        lsx_option_arg_required, NULL, 0 }, { "help-format",
        lsx_option_arg_required, NULL, 0 }, { "no-glob", lsx_option_arg_none,
        NULL, 0 }, { "plot", lsx_option_arg_required, NULL, 0 }, {
        "replay-gain", lsx_option_arg_required, NULL, 0 }, { "version",
        lsx_option_arg_none, NULL, 0 }, { "output", lsx_option_arg_required,
        NULL, 0 }, { "effects-file", lsx_option_arg_required, NULL, 0 }, {
        "temp", lsx_option_arg_required, NULL, 0 }, { "single-threaded",
        lsx_option_arg_none, NULL, 0 }, { "ignore-length", lsx_option_arg_none,
        NULL, 0 }, { "norm", lsx_option_arg_optional, NULL, 0 }, { "magic",
        lsx_option_arg_none, NULL, 0 }, { "play-rate-arg",
        lsx_option_arg_required, NULL, 0 }, { "clobber", lsx_option_arg_none,
        NULL, 0 }, { "no-clobber", lsx_option_arg_none, NULL, 0 }, {
        "multi-threaded", lsx_option_arg_none, NULL, 0 }, { "dft-min",
        lsx_option_arg_required, NULL, 0 },

{ "bits", lsx_option_arg_required, NULL, 'b' }, { "channels",
        lsx_option_arg_required, NULL, 'c' }, { "compression",
        lsx_option_arg_required, NULL, 'C' }, { "default-device",
        lsx_option_arg_none, NULL, 'd' }, { "no-dither", lsx_option_arg_none,
        NULL, 'D' }, { "encoding", lsx_option_arg_required, NULL, 'e' }, {
        "help", lsx_option_arg_none, NULL, 'h' }, { "null", lsx_option_arg_none,
        NULL, 'n' }, { "no-show-progress", lsx_option_arg_none, NULL, 'q' }, {
        "pipe", lsx_option_arg_none, NULL, 'p' }, { "rate",
        lsx_option_arg_required, NULL, 'r' }, { "reverse-bits",
        lsx_option_arg_none, NULL, 'X' }, { "reverse-nibbles",
        lsx_option_arg_none, NULL, 'N' }, { "show-progress",
        lsx_option_arg_none, NULL, 'S' }, { "type", lsx_option_arg_required,
        NULL, 't' }, { "volume", lsx_option_arg_required, NULL, 'v' }, {
...
static lsx_enum_item const encodings[] = { { "signed-integer",
        encoding_signed_integer }, { "unsigned-integer",
        encoding_unsigned_integer },
        { "floating-point", encoding_floating_point }, { "ms-adpcm",
                encoding_ms_adpcm }, { "ima-adpcm", encoding_ima_adpcm }, {
                "oki-adpcm", encoding_oki_adpcm }, { "gsm-full-rate",
                encoding_gsm_full_rate }, { "u-law", encoding_u_law }, {
                "mu-law", encoding_u_law }, { "a-law", encoding_a_law },
        { 0, 0 } };

static int enum_option(char const * arg, int option_index,
        lsx_enum_item const * items) {
...
#define SOX_BUFMIN 16
                if (sscanf(optstate.arg, "%i %c", &i, &dummy)
                        != 1|| i <= SOX_BUFMIN) {
                    lsx_fail("Buffer size `%s' must be > %d", optstate.arg,
                            SOX_BUFMIN);
                    exit(1);
                }
            ...
            case 5:
                ...
                switch (enum_option(optstate.arg, optstate.lngind,
                        endian_options)) {
                case ENDIAN_little:
            ...
            case 10:
                ...
                break;
            case 11:
                sox_effects_globals.plot = enum_option(optstate.arg,
                        optstate.lngind, plot_methods);
                break;
            case 12:
                replay_gain_mode = enum_option(optstate.arg, optstate.lngind,
                        rg_modes);
                break;
            case 13:
                display_SoX_version (stdout);
            ...
            case 24:
                ...
                break;
            case 25:
                if (sscanf(optstate.arg, "%i %c", &i, &dummy) != 1 || i < 8
                        || i > 16) {
        ...
        case 'r': {
            ...
            if (n < 1 || f->signal.rate <= 0 || (n > 1 && k != 'k') || n > 2) {
                lsx_fail("Rate value `%s' is not a positive number",
                        optstate.arg);
                exit(1);
            }
        ...
        case 'c':
            if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i <= 0) {
                lsx_fail("Channels value `%s' is not a positive integer",
                        optstate.arg);
                exit(1);
            }
            f->signal.channels = i;
        ...
        case 'C':
            if (sscanf(optstate.arg, "%lf %c", &f->encoding.compression, &dummy)
                    != 1) {
                lsx_fail("Compression value `%s' is not a number",
                        optstate.arg);
                exit(1);
            }
            break;
        ...
        case 'b':
            if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i <= 0) {
                lsx_fail("Bits value `%s' is not a positive integer",
                        optstate.arg);
                exit(1);
            }
...
static char const * device_name(char const * const type) {
    ...
    if (0 || !strcmp(type, "sunau") || !strcmp(type, "oss")
            || !strcmp(type, "ossdsp") || !strcmp(type, "alsa")
            || !strcmp(type, "ao") || !strcmp(type, "sndio")
...
static void parse_options_and_filenames(int argc, char **argv) {
    ...
    if (env_opts && *env_opts) {
        char * * argv2, *str = lsx_malloc(
                strlen(argv[0]) + strlen(env_opts) + 2);
        int argc2;
        strcpy(str, argv[0]);
        strcat(str, " ");
        strcat(str, env_opts);
        argv2 = strtoargv(str, &argc2);
        lsx_getopt_init(argc2, argv2, getoptstr, long_options,
                lsx_getopt_flag_opterr, 1, &optstate);
    ...
    lsx_getopt_init(argc, argv, getoptstr, long_options, lsx_getopt_flag_opterr,
            1, &optstate);
...
static int soxi(int argc, char * const * argv) {
    ...
    if (argc < 2)
        soxi_usage(0);
    lsx_getopt_init(argc, argv, opts, NULL, lsx_getopt_flag_opterr, 1,
            &optstate);
    ...
    for (; optstate.ind < argc; ++optstate.ind) {
        if (sox_is_playlist(argv[optstate.ind]))
            num_errors += (sox_parse_playlist((sox_playlist_callback_t) soxi1,
                    &type, argv[optstate.ind]) != SOX_SUCCESS);
        else
...
static void output_message(unsigned level, const char *filename,
    ...
    if (sox_globals.verbosity >= level) {
        ...
        fprintf(stderr, "%s %s %s: ", myname, str[min(level - 1, 3)],
                base_name);
    ...
    if (replay_gain_mode == RG_default)
        replay_gain_mode = is_player ?
                        ...
                        cmp_comment_text(
                            ...
                            sox_find_comment(
                                files[0]->ft->oob.comments,
                                "artist"),
                            sox_find_comment(
                                files[1]->ft->oob.comments,
                                "artist"))
                        && cmp_comment_text(
                            sox_find_comment(
                                files[0]->ft->oob.comments,
